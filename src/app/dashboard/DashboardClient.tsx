...
export default function DashboardClient({ workflows: initial }: Props) {
  ...
  const createWorkflow = async () => {
    ...
    try {
      ...
      if (!data.id) { setCreating(false); return }
          ...
          <div className="bg-[#141414] rounded-xl p-4 mb-8 border border-white/5">
            <div className="flex gap-4 mb-4">
              <button
                ...
                className={`text-sm font-medium pb-1 border-b-2 transition-colors ${activeTab === "workflows" ? "border-white text-white" : "border-transparent text-white/40 hover:text-white/60"}`}
              >
                Workflow library
              </button>
              <button
                onClick={() => setActiveTab("tutorials")}
                className={`text-sm font-medium pb-1 border-b-2 transition-colors ${activeTab === "tutorials" ? "border-white text-white" : "border-transparent text-white/40 hover:text-white/60"}`}
              >
                Tutorials
              </button>
            </div>
            <div className="flex gap-3 overflow-x-auto pb-2">
              {sampleWorkflows.map((sw, i) => (
                <div key={i} className="flex-shrink-0 w-44 h-28 rounded-lg overflow-hidden relative cursor-pointer group">
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-1 bg-[#1a1a1a] border border-white/10 rounded-lg p-1">
                  <button onClick={() => setView("list")} className={`p-1 rounded transition-colors ${view === "list" ? "bg-white/20" : "hover:bg-white/10"}`}>
                    <List size={14} />
                  </button>
                  <button onClick={() => setView("grid")} className={`p-1 rounded transition-colors ${view === "grid" ? "bg-white/20" : "hover:bg-white/10"}`}>
                    <LayoutGrid size={14} />
                  </button>
                </div>
              </div>
            ...
            {filtered.length === 0 ? (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                {filtered.map((w) => (
                  <div key={w.id} onClick={() => router.push(`/workflow/${w.id}`)} className="group cursor-pointer">
                    ...
                    <p className="text-xs text-white/30 mt-0.5">
                      Last edited {formatDistanceToNow(new Date(w.updatedAt))} ago
                    </p>
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-2">
                {filtered.map((w) => (
                  <div key={w.id} onClick={() => router.push(`/workflow/${w.id}`)} className="flex items-center gap-4 p-3 bg-[#1a1a1a] rounded-lg border border-white/5 hover:border-white/20 cursor-pointer transition-colors">
